...
// Chart placeholder component with enhanced loading states
const ChartPlaceholder = React.memo(({ title, height = 200, isLoading = false }) => (
  <div 
    className={`bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg flex items-center justify-center border-2 border-dashed border-blue-200 transition-all duration-300 ${
    ...
    <div className="text-center">
      {isLoading ? (
        <div className="space-y-2">
          <LoadingSkeleton className="w-8 h-8 mx-auto rounded" />
...
// Metric card component with enhanced loading and animations
const MetricCard = React.memo(({ 
  ...
  return (
    <div className="bg-white rounded-lg p-6 border border-gray-200 hover:border-gray-300 transition-all duration-200 card-hover animate-fadeIn">
      <div className="flex items-center justify-between">
        ...
        {change !== undefined && (
          <div className={`flex items-center text-sm ${
            ...
            <TrendingUp size={14} className={`mr-1 transition-transform duration-200 ${trend === 'down' ? 'rotate-180' : ''}`} />
...
// Leave trends component with preload support
const LeaveTrends = React.memo(() => {
  ...
  return (
    <div className="bg-white rounded-lg p-6 border border-gray-200 animate-slideUp">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-gray-900">Leave Trends</h3>
        <select
          value={timeRange}
          onChange={(e) => setTimeRange(e.target.value)}
          className="px-3 py-1 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-600 focus:border-transparent transition-colors"
        >
      ...
      {trendData ? (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-blue-50 rounded-lg animate-fadeIn">
              <p className="text-2xl font-bold text-blue-600">{trendData.totalLeaves}</p>
              <p className="text-sm text-blue-600">Total Requests</p>
            </div>
            
            <div className="text-center p-4 bg-green-50 rounded-lg animate-fadeIn animation-delay-100">
              <p className="text-2xl font-bold text-green-600">{trendData.totalDays}</p>
              <p className="text-sm text-green-600">Total Days</p>
            </div>
            
            <div className="text-center p-4 bg-purple-50 rounded-lg animate-fadeIn animation-delay-200">
            ...
            <div className="text-center p-4 bg-orange-50 rounded-lg animate-fadeIn animation-delay-300">
          ...
          <div className="animate-fadeIn animation-delay-200">
            ...
            <div className="space-y-2">
              {Object.entries(trendData.leaveTypes).map(([type, days], index) => (
                <div 
                  key={type} 
                  className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors animate-fadeIn"
...
// Team performance insights with enhanced loading
          ...
          {[...Array(3)].map((_, i) => (
            <div key={i} className="p-4 rounded-lg border-l-4 border-gray-200 bg-gray-50">
              <div className="flex items-start space-x-3">
                <LoadingSkeleton className="w-5 h-5 mt-0.5" />
      ...
      {insights.length > 0 ? (
        <div className="space-y-4">
          {insights.map((insight, index) => (
            <div
              key={index}
              className={`p-4 rounded-lg border-l-4 transition-all duration-300 hover:shadow-sm animate-fadeIn ${
...
// Main analytics component with preload support
const TeamAnalyticsWithPreload = React.memo(() => {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Key Metrics with staggered loading */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg p-6 border border-gray-200 animate-slideUp">
          ...
          <ChartPlaceholder title="Monthly Leave Calendar View" height={300} isLoading={!isDataReady} />
        </div>
        
        <div className="bg-white rounded-lg p-6 border border-gray-200 animate-slideUp animation-delay-100">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Popular Destinations</h3>
          <ChartPlaceholder title="Team Travel Destinations Map" height={300} isLoading={!isDataReady} />
        </div>
      </div>
...
TeamAnalyticsWithPreload.displayName = 'TeamAnalyticsWithPreload';

export default TeamAnalyticsWithPreload;
